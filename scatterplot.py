...
def images(projection, images, glyph_width, glyph_height, label=None,
           alpha=1.0, figsize=(10, 10), fontsize=6, cmap='gray'):
    """
    Desenha cada imagem na posição overlap-free definida pelo grid/projection.
    images: numpy array (N, H, W)
    projection: array (N, 2)
    """
    max_glyph_size = max(glyph_width, glyph_height)
    max_coordinates = np.amax(projection, axis=0)
    min_coordinates = np.amin(projection, axis=0)
    ...
    for i in range(len(projection)):
        x_, y_ = projection[i]
        img = images[i]
        # Desenha imagem centralizada em (x_, y_)
        ax.imshow(img, cmap=cmap, alpha=alpha,
                  extent=(x_ - glyph_width/2, x_ + glyph_width/2,
                          y_ - glyph_height/2, y_ + glyph_height/2),
                  interpolation='nearest')
        if label is not None:
            ax.text(x_, y_ - glyph_height/2 - 2, str(label[i]), color='black',
